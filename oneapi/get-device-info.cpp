...
int main() {
  ...
  try
  {
    ...
    //
    cl::sycl::device parent_device;
    try {
      parent_device = device.get_info<info::device::parent_device>();
      std::cout << "parent_device_name: " << parent_device.get_info<info::device::name>() << std::endl;
    }
    catch(sycl::exception& e)
    {
      std::cout << "ERROR (RECOVERABLE): When trying to get a parent device caught synchronous exception => " << std::endl << "  " << e.what() << std::endl;
    }
    //
  ...
  {
    std::cout << "ERROR (ABORT): When trying to initialize a queue with selector " << typeid(selector).name() << " caught synchronous exception => " << std::endl << "  " << e.what() << std::endl;
    return 1;
  }
  return 0;
}
